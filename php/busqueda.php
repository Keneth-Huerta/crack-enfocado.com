    ...
    <?php if ($resultado_usuarios && mysqli_num_rows($resultado_usuarios) > 0): ?>
        <ul>
            <?php while ($usuario = mysqli_fetch_assoc($resultado_usuarios)): ?>
                <li>
                    <img src="<?php echo htmlspecialchars($usuario['foto_perfil']); ?>" alt="Foto de perfil" width="40" height="40">
                    <?php echo htmlspecialchars($usuario['nombre'] . ' ' . $usuario['apellido']); ?> - <?php echo htmlspecialchars($usuario['username']); ?>
                </li>
            <?php endwhile; ?>
        </ul>
    <?php else: ?>
