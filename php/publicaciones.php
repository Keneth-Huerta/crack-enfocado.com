                ...
                <?php while ($publicacion = mysqli_fetch_assoc($publicaciones_result)) { ?>
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <?php
                            $comentarios_query = "SELECT * FROM comentarios WHERE publicacion_id = ?";
                            $stmt_com = mysqli_prepare($enlace, $comentarios_query);
                            mysqli_stmt_bind_param($stmt_com, "i", $publicacion['id_publicacion']);
                            mysqli_stmt_execute($stmt_com);
                            $comentarios_result = mysqli_stmt_get_result($stmt_com);
                            while ($comentario = mysqli_fetch_assoc($comentarios_result)) {
                                echo '<p><strong>' . htmlspecialchars($comentario['usuario']) . ':</strong> ' . htmlspecialchars($comentario['contenido']) . '</p>';
                            }
